...
function ChildScrollViewContent.onRefreshItemView(container)
    if container.mScrollView == nil then
        ...
        if container.ParentContainer ~= nil then
            --调整scrollview位置和大小和横向滑动scrollview一致
            container.mScrollView:setViewSize(container.ParentContainer.mScrollView:getViewSize())
...
function MultiColumnScrollViewHelper.buildScrollViewHorizontal(container, buildTable, interval)
    if buildTable == nil or container == nil or container.mScrollView == nil or (#buildTable) < 1 then
        return
    end
    --初始化数据
...
function MultiColumnScrollViewHelper.onTouchMove(container, eventName, pTouch)
	...
	if not container.m_BegainX or not container.m_BegainY then
		return 
	end    
    ...
    if container.maxScrollWidth == nil then
        container.maxScrollWidth = container.mScrollView:getContentSize().width
    end
    if container.fOneItemWidth == nil then
        local children = container.mScrollViewRootNode:getChildren()
        if children:count() > 0 then
            local child = tolua.cast(children:objectAtIndex(0), "CCNode")
            container.fOneItemWidth = child:getContentSize().width
        end
    end
    if container.ScrollSize == nil then
        local children = container.mScrollViewRootNode:getChildren()
        container.ScrollSize = children:count()
    end
...
function MultiColumnScrollViewHelper.onTouchEnd(container, eventName, pTouch)
    ...
    if container.mCanHScroll then
        local ceilWidth = container.fOneItemWidth or 20
        local interval = container.ScrollInterval or 20
        local size = container.ScrollSize ~= 0 and container.ScrollSize or 1
