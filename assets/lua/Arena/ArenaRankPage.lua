...
function ArenaRankPageContent.onRefreshItemView(container)
	...
	local lb2Str = {
		mLv 			= common:getR2LVL() .. itemInfo.level,
	...
	NodeHelper:setLabelOneByOne(container,"mFightingLable","mFightingNum",5)
    NodeHelper:setLabelOneByOne(container,"mGuildlabel","mGuildName",5)
 	NodeHelper:setLabelOneByOne(container,"mRankingLabel","mRankingNum",5)
	...
	if itemInfo.identityType == 2 then
	    ...
	    --    headPic = roleConfig[temInfo.cfgItemId]["wingIcon"]
        --else
            headPic = roleConfig[temInfo.cfgItemId]["icon"]
        --end
	else
