...
--»î¶¯»ù±¾ÐÅÏ¢£¬½á¹¹»ù±¾ÓëÐ­Òépb MessageÏàÍ¬
...
function ConsumeItem.onRefreshItemView(container)
	...
	if thisActivityInfo.gotIds[itemCfgId] then
...
-----------------------------------------------
--AccumulativeConsumeBaseÒ³ÃæÖÐµÄÊÂ¼þ´¦Àí
----------------------------------------------
function AccumulativeConsumeBase:onEnter(container)
	...
	if Golb_Platform_Info.is_r2_platform and IsThaiLanguage() then
		NodeHelper:MoveAndScaleNode(container, { mRechargeConsumptionPageTitle = common:getLanguageString("@Act_RechargeConsumptionPage_Title") },
			0, 0.8)
	end
end
...
function AccumulativeConsumeBase:clearPage(container)
	NodeHelper:setStringForLabel(container, { mActivityDaysNum = "", mConsumptionGoldNum = "" })
end
...
function AccumulativeConsumeBase:onRecharge(container)
	libPlatformManager:getPlatform():sendMessageG2P("G2P_ENTER_RECHARGE_PAGE", "AccumulativeConsume_enter_recharge")
