...
function ExpeditionRankPageBase:onEnter(container)
    ...
    if Golb_Platform_Info.is_r2_platform and IsThaiLanguage() then
       NodeHelper:MoveAndScaleNode(container,{mExpeditionMaterialsCollectionRankTitle = common:getLanguageString("@Act_ExpeditionMaterialsCollectionRankPopUp_Title")},0,0.7);
    end
...
function ExpeditionRankPageBase:refreshPage(container)
    ...
    if Golb_Platform_Info.is_r2_platform then
	    NodeHelper:setStringForLabel(container, {mContributionTex = common:getLanguageString("@Contribution_ACT_Expedition_R2")})
	 end
