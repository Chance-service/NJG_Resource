...
local thisPageName = 'GoodsExchangePage'
...
function GoodsExchangePage.onFunction(eventName, container)
    if eventName == "onBuyAll" then
        GoodsExchangePage:onBuyAll();
    elseif eventName == "onOpenUi" then
        GoodsExchangePage:onPreview();
    elseif eventName == "onBrushSuit" then
        GoodsExchangePage:onBrushSuit();
    end
end
function GoodsExchangePage:onBuyAll()
...
function GoodsExchangePage:onEnter(ParentContainer, index)
    _IsClickFlag = false
    self.container = ScriptContentBase:create(GoodsExchangePage.ccbiFile)
    self.container:registerFunctionHandler(GoodsExchangePage.onFunction)
    ...
    GoodsExchangePage:getPageInfo(index)

    return self.container
end


function GoodsExchangePage:refreshPage()
    -- self:rebuildAllItem()
    GoodsExchangePage:rebuildAllTitleItem(self.container)
end

function GoodsExchangePage:onExecute(ParentContainer)
    self:onTimer();
end

function GoodsExchangePage:onTimer(container)
    if ExchangeActivityInfo.timeLeft < 0 then
        return;
    end
    local timerName = GoodsExchangePage.timerName;
    ...
    if TimeCalculator:getInstance():hasKey(timerName) then
        ...
        if ExchangeActivityInfo.timeLeft > 0 then
            ...
            TimeCalculator:getInstance():removeTimeCalcultor(GoodsExchangePage.timerName);
        end
...
function GoodsExchangePage:onHandTileClick(container)
    _IsClickFlag = true
    local index = container:getItemDate().mID;
    GoodsExchangePage:selectTileHand(index)
end
function GoodsExchangePage:selectTileHand(index)
...
function ItemTitleContent.onFunction(eventName, container)
    ...
    elseif eventName == "onHand" then
        GoodsExchangePage:onHandTileClick(container)
    end
end
...
function ItemTitleContent:onRefreshContent(container)
    ...
    local menu2Quality = { };
    local colorMap = { }
    local consumeCfg = GoodsExchangePage:splitTiem(itemInfo.consumeInfo)
    ...
    if index == common:getTableLen(ExchangeActivityItemId) then
        if _SelectIndex == 0 then
            GoodsExchangePage:selectTileHand(1)
        else
            GoodsExchangePage:selectTileHand(_SelectIndex)
        end

    end
end

function GoodsExchangePage:rebuildAllTitleItem(container)
    ...
    container.mCostScrollView:setTouchEnabled(false)
    ...
    GoodsExchangePage:clearAllTitleItem(container)
    local maxSize = #ExchangeActivityItemId
    GoodsExchangePage:buildScrollViewHorizontal(self.container, maxSize, ItemTitleContent.ccbiFileCost, ItemTitleContent.onFunction);
end

function GoodsExchangePage:buildScrollViewHorizontal(container, size, ccbiFile, funcCallback, interval)
...
function GoodsExchangePage:splitTiem(itemInfo)
    local items = { }
...
function GoodsExchangePage:getExchangeCount(index)
...
function ItemContent:onRefreshContent(ccbRoot)
    ...
    if itemInfo then
        local consumeCfg = GoodsExchangePage:splitTiem(itemInfo.consumeInfo)
        local awardCfg = GoodsExchangePage:splitTiem(itemInfo.awardInfo)
        local lb2Str = { };
        local sprite2Img = { };
        local menu2Quality = { };
        local colorMap = { };
        ...
        lb2Str["mName"] = resInfo.name;
        colorMap["mName"] = ConfigManager.getQualityColor()[resInfo.quality].textColor
        lb2Str["mCommodityNum"] = "x" .. consumeCfg[1].count;
        ...
        local exchangeCount = GoodsExchangePage:getExchangeCount(itemInfo.id)
...
function ItemContent:onBuy(container)
    ...
    if ActivityData.SysBasic.ForgedStoneItemId == _SelectItemId and list[index].roleId ~= 0 then
        ...
        --        ExclusiveEquipment:setDataInfo(list[index].roleId,GoodsExchangePage.updateCallback)
    ...
    local consumeCfg = GoodsExchangePage:splitTiem(itemInfo.consumeInfo)
    -- 兑换的道具
    local awardCfg = GoodsExchangePage:splitTiem(list[index].awardInfo)
    -- 已兑换次数
    local exchangeCount = GoodsExchangePage:getExchangeCount(itemInfo.id)
    -- 剩余兑换次数
    local remainTime = itemInfo.maxExchangeTime - exchangeCount
    ...
    -- 消耗道具
    local consumeCfg = GoodsExchangePage:splitTiem(list[index].consumeInfo)
    local awardCfg = GoodsExchangePage:splitTiem(list[index].awardInfo)
    ...
    local exchangeCount = GoodsExchangePage:getExchangeCount(id)
    ...
    if maxCount <= 0 then
        ...
        local msg = Language:getInstance():getString("@NotEnoughExchangeItem")
        PageManager.showConfirm(title, msg, function(isSure)
            if isSure then
                GoodsExchangePage.jumpToActivity_101()
            end
        end )
        return
    end
...
function GoodsExchangePage.jumpToActivity_101()
...
function GoodsExchangePage.updateCallback()
...
function GoodsExchangePage:rebuildAllItem()
    self:clearAllItem();
    self:buildItem();
end

function GoodsExchangePage:clearAllItem()
...
function GoodsExchangePage:buildItem()
...
function GoodsExchangePage:getPageInfo(index)
    ExchangeActivityCfg = { }
    ...
    for k, v in pairs(ExchangeActivityCfg.cfg) do
        local itemInfo = v
        local consumeCfg = GoodsExchangePage:splitTiem(itemInfo.consumeInfo)[1]
...
function GoodsExchangePage:refreshUserItemCount()
    for i = 1, #ExchangeActivityItemId do
...
function GoodsExchangePage:getActivityInfo()
    common:sendEmptyPacket(opcodes.EXCHANGE_INFO_C, false)
end

function GoodsExchangePage:onReceivePacket(ParentContainer)
...
function GoodsExchangePage:handleAcitivityInfo(msg)
    ExchangeActivityInfo.timeLeft = msg.lastCount
    if ExchangeActivityInfo.timeLeft > 0 and not TimeCalculator:getInstance():hasKey(GoodsExchangePage.timerName) then
        TimeCalculator:getInstance():createTimeCalcultor(GoodsExchangePage.timerName, ExchangeActivityInfo.timeLeft);
    end
...
function GoodsExchangePage:registerPacket(ParentContainer)
...
function GoodsExchangePage:removePacket(ParentContainer)
    for key, opcode in pairs(opcodes) do
        if string.sub(key, -1) == "S" then
...
function GoodsExchangePage:clearAllTitleItem(container)
...
function GoodsExchangePage:deleteAllTitleItem(container)
...
function GoodsExchangePage:onExit(ParentContainer)
    self:clearAllItem();
    NodeHelper:deleteScrollView(self.container)
    TimeCalculator:getInstance():removeTimeCalcultor(GoodsExchangePage.timerName);
    self:removePacket(ParentContainer);
    GoodsExchangePage:deleteAllTitleItem(self.container)
    UserItemCountList = { }
    ExchangeActivityCfg = { }
