...
function RegimentWarControlItem.onRefreshItemView(container)
	...
	if memberInfo ~= nil then
		...
		for i = 1, maxSize, 1 do
			...
			local skillLv = memberInfo.skillLevel[i]
			if skillCfg[skillId] ~= nil then
				--[[
                local SkillCfg = SEManager:getSECfgByProfession(roleConfig[memberInfo.roleCfgId].profession)
				if not SkillCfg[skillId] then
					CCLuaLog("skillCfg[skillId][] == nil"..skillId)
					break
				end]]
                local skillItemId = skillId--SkillCfg[skillId].skillItemId
                local cfgItem = SEManager:getSkillCfgInfoByItemId(skillItemId,skillLv)
				if cfgItem then
					local skillPic = cfgItem["icon"];
	...
	if memberInfo ~= nil then
		...
		if kickObj ~= nil then
			local kickPlayerFunc = function ( isOk )
				if isOk and UserInfo.isGoldEnough(goldCost,"kickPlayer_enter_rechargePage") then
...
function RegimentWarControlBase:onEnter(container)
	...
	NewbieGuideManager.showHelpPage(GameConfig.HelpKey.HELP_REGIMENTWAR_TEAM)
end
...
function RegimentWarControlBase:onReceiveTeamList( container, msg )
	...
	for i = 1,memberSize,1 do
        ...
        TeamList[i].skillLevel = memberInfo.skillLevel;
