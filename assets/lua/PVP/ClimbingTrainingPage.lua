...
local thisPageName = "SoulStarPage"
...
function SoulStarPageBase:onEnter(container)
    ...
    -- node:addChild(itemNode);
    -- itemNode:registerFunctionHandler(SoulStarPageBase.onFunction)
...
function SoulStarPageBase:getAttr(id)
...
function SoulStarPageBase:onReceivePacket(container)
...
function SoulStarPageBase:onImmediatelyDekaron(container)
...
function SoulStarPageBase_setRedPoint(marks)
    --    for i = 1, 6 do
    --        if btnCCBs[i] then
...
local SoulStarPage = CommonPage.newSub(SoulStarPageBase, thisPageName, option)