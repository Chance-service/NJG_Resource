...
function RegimentWarPageBase:onEnter(container)
	...
	self:refreshPage(container);
	NewbieGuideManager.showHelpPage(GameConfig.HelpKey.HELP_REGIMENTWAR)
    if IsThaiLanguage() then
        NodeHelper:MoveAndScaleNode(container, { mRegimentWarTex1 = common:getLanguageString("@RegimentWarTex1")},0,0.65,0.65);
        NodeHelper:MoveAndScaleNode(container, { mRegimentWarTex2 = common:getLanguageString("@RegimentWarTex2")},0,0.65,0.65);
        NodeHelper:MoveAndScaleNode(container, { mRegimentWarTex3 = common:getLanguageString("@RegimentWarTex3")},0,0.65,0.65);
        NodeHelper:setNodeScale(container, "mExplainTex", 0.65, 0.65)
    end
end
...
function RegimentWarPageBase:onReturn(container)
...
function RegimentWarPageBase:createTeam(container)
	...
	local createTeamFunc = function ( isOk )
		if isOk and UserInfo.isGoldEnough(GameConfig.Cost.CreateRegimentTeam,"CreateRegimentTeam_enter_rechargePage") then
...
function RegimentWarPageBase:onReceiveBattleInfo( container, msg )
	...
	TimeCalculator:getInstance():createTimeCalcultor("RegimentWarCDActivity", PageInfo.BattleLeftTime);
