...
function CampFightPageBase:onReturn(container)
...
function CampFightPageBase:refreshPersonalStatic(container)
	...
	NodeHelper:setStringForLabel(container,{
		mLevelNum = UserInfo.getStageAndLevelStr()
	})
...
function CampFightPageBase:refreshRankPanel(container)
	...
	if #CampWarManager.m_curCampFightInfo.rankInfo == 0 then
		...
		for i=1,3 do
			...
			if i<=rankSize then			
				if container:getVarNode(nodeStr) then	
					container:getVarNode(nodeStr):setVisible(true)
				end
				--基本信息	
				...
				if Golb_Platform_Info.is_r2_platform then
					if killNode then killNode:setVisible(false) end
					labelMap[killNumStr] = common:getLanguageString("@KillNum", tostring(CampWarManager.getKillRankInfoByRank(i).multiKillQty))
				else
				...
				if container:getVarNode(nodeStr) then
					container:getVarNode(nodeStr):setVisible(false)
				end
			end								
		end			
	end
...
function CampFightPageBase:onShowNo1(container)
	if CampWarManager.getKillRankInfoByRank(1) == nil then return end
...
function CampFightPageBase:onShowNo2(container)
	if CampWarManager.getKillRankInfoByRank(2) == nil then return end
...
function CampFightPageBase:onShowNo3(container)
	if CampWarManager.getKillRankInfoByRank(3) == nil then return end
