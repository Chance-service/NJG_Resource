...
local opcodes = {
    ACTIVITY167_FREE_SUMMOM_S=HP_pb.ACTIVITY167_FREE_SUMMOM_S,
...
function DayLogin30Item:onAutoClick()
    if self.mState == DayLogin30ItemState.CanGet then
        local msg = Activity5_pb.FreeSummonReq()
        msg.action = 1
        common:sendPacket(HP_pb.ACTIVITY167_FREE_SUMMON_C, msg, false)
    end
end
...
function DayLogin30Page:onEnter(container)
    ...
    NodeHelper:setNodesVisible(container,{mOpenNode = false,mBannerCost = false,mBannerFree = true})
     NodeHelper:setSpriteImage(container,{mBanner = "900 Summons_Text.png"})
...
function DayLogin30Page:initData()
    ...
    configData = ConfigManager.getFreeSummonData()
...
function DayLogin30Page:setSeverData(msg)
     serverData = { }
...
function DayLogin30Page:onReceivePacket(packet)
    ...
    if opcode == HP_pb.ACTIVITY167_FREE_SUMMOM_S then
...
function DayLogin30Page:hasData()
   return serverData.monthOfDay ~= nil
end

function DayLogin30Page:AllSigned()
   return serverData.signedDays == #ConfigManager.getFreeSummonData() 
end
--
