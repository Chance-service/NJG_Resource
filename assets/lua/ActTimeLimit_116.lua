...
local thisPageName = "ActTimeLimit_116"
...
local mLastMoveItem = { }
local option = {
    ccbiFile = "Act_TimeLimit_116.ccbi",
    handlerMap =
    {
...
local opcodes = {
    -- 请求界面信息
    RELEASE_UR_INFO2_C = HP_pb.RELEASE_UR_INFO2_C,
    -- 界面信息返回
    RELEASE_UR_INFO2_S = HP_pb.RELEASE_UR_INFO2_S,
    -- 抽奖请求
    RELEASE_UR_DRAW2_C = HP_pb.RELEASE_UR_DRAW2_C,
    -- 抽奖返回
    RELEASE_UR_DRAW2_S = HP_pb.RELEASE_UR_DRAW2_S,
    -- 积分抽奖请求
    RELEASE_UR_LOTTERY2_C = HP_pb.RELEASE_UR_LOTTERY2_C,
    -- 积分抽奖返回
    RELEASE_UR_LOTTERY2_S = HP_pb.RELEASE_UR_LOTTERY2_S,
    -- 副将信息请求
...
ActTimeLimit_116.timerName = "Activity_ActTimeLimit_116"
ActTimeLimit_116.timerLabel = "mTanabataCD"
ActTimeLimit_116.timerKeyBuff = "Activity_ActTimeLimit_116_Timer_Key_Buff"
ActTimeLimit_116.timerFreeCD = "Activity_ActTimeLimit_116_Timer_Free_CD"

function ActTimeLimit_116:onEnter(parentContainer)
    math.randomseed(os.time())
    local container = ScriptContentBase:create(option.ccbiFile)
    self.container = container
    luaCreat_ActTimeLimit_116(container)
...
function ActTimeLimit_116:initData()
    mCurrentMoveIndex = 1
    ...
    -- 本期抽奖可以获得的副将
    _MercenaryInfo = ConfigManager.getReleaseURdrawMercenary116Cfg()[1]
    -- 本期抽奖可以获得的奖励
    TreasureRaiderDataHelper.TreasureRaiderConfig = ConfigManager.getReleaseURdrawReward116Cfg() or { }
...
function ActTimeLimit_116:initPosIndexData()
    mPosIndexData = { }
    for i = 1, 9 do
...
function ActTimeLimit_116:initUi(container)
...
-- 得到下一个移动到的位置索引
function ActTimeLimit_116:getPosIndex(index)
...
-- 添加移动位置索引
function ActTimeLimit_116:addPosIndex(index)
    table.insert(mPosIndexData, index)
end

function ActTimeLimit_116:setRollItemData(item, data, index)
...
function ActTimeLimit_116:getPageInfo(container)
    -- 请求界面信息
    common:sendEmptyPacket(HP_pb.RELEASE_UR_INFO2_C)
    --- 请求副将信息
    common:sendEmptyPacket(HP_pb.ROLE_PANEL_INFOS_C, false)
end

-- 刷新UR抽奖界面
function ActTimeLimit_116:refreshURPage(container)
...
function ActTimeLimit_116:initScoreRewardItemPosition()
...
-- 刷新积分抽奖按钮状态
function ActTimeLimit_116:refreshScoreBtn(container)
    local bl = true
...
-- 刷新页面
function ActTimeLimit_116:refreshPage(container)
    self:refreshURPage(container)
end

-- 收包
function ActTimeLimit_116:onReceivePacket(parentContainer)
    ...
    elseif opcode == HP_pb.RELEASE_UR_INFO2_S or opcode == HP_pb.RELEASE_UR_DRAW2_S then
        -- 界面信息返回
        msg = Activity3_pb.ReleaseURInfo2()
        msg:ParseFromString(msgBuff)
        self:updateData(parentContainer, opcode, msg)

    elseif opcode == HP_pb.RELEASE_UR_LOTTERY2_S then
        -- 积分抽奖返回
        msg = Activity3_pb.ReleaseURLotteryRep2()
        msg:ParseFromString(msgBuff)
...
-- 显示积分抽奖过程
function ActTimeLimit_116:showScoreAnimation()
...
-- 正常动画
function ActTimeLimit_116:showAnimation()
...
-- 结束动作
function ActTimeLimit_116:endAnimation()
...
-- 结果动作
function ActTimeLimit_116:resultAnimation()
    -- TODO   重置当前的位置信息
...
function ActTimeLimit_116:AAAAAA()
    local t = { }
    for k, v in pairs(mScorePoolItem) do
        local itemTag = v:getTag()
        local itemIndex = math.modf(itemTag / 10)
...
function ActTimeLimit_116:getTargetPosIndex()
...
function ActTimeLimit_116:getEndPosIndex(index)
...
function ActTimeLimit_116:popUpRewardPage()
...
-- 刷新积分
function ActTimeLimit_116:refreshScore(comluckey)
    if comluckey < 0 then
        comluckey = 0
    end
    local label2Str = {
...
function ActTimeLimit_116:updateData(parentContainer, opcode, msg)
    ...
    if opcode == HP_pb.NEW_TREASURE_RAIDER_INFO_S or opcode == HP_pb.RELEASE_UR_INFO2_S then

    elseif opcode == HP_pb.NEW_TREASURE_RAIDER_SEARCH_S or opcode == HP_pb.RELEASE_UR_DRAW2_S then
        common:sendEmptyPacket(HP_pb.ROLE_PANEL_INFOS_C, false)
...
function ActTimeLimit_116:pushRewardPage()
    local data = { }
    ...
    if not CommonRewardAni:isPop() then
        ...
        CommonRewardAni:setFirstDataNew(data.onceGold, data.tenGold, data.rewards, isFree, 0, "", false, ActTimeLimit_116.onBtnClick_1, ActTimeLimit_116.onBtnClick_2, function()
        ...
        CommonRewardAni:setFirstDataNew(data.onceGold, data.tenGold, data.rewards, isFree, 0, "", true, ActTimeLimit_116.onBtnClick_1, ActTimeLimit_116.onBtnClick_2)
    end
end



function ActTimeLimit_116:isContain(t, n)
...
function ActTimeLimit_116:registerPacket(container)
...
function ActTimeLimit_116:removePacket(container)
    for key, opcode in pairs(opcodes) do
        if string.sub(key, -1) == "S" then
...
function ActTimeLimit_116:onExecute(parentContainer)
    self:onTimer(self.container)
end

function ActTimeLimit_116:onTimer(container)
...
function ActTimeLimit_116:onExit(parentContainer)
    local node = self.container:getVarNode("mRewardNode")
    if node then
        node:removeAllChildren()
    end
...
function ActTimeLimit_116:initSpine(container)
    local spineNode = container:getVarNode("mSpineNode");
    -- local spinePosOffset = "-100,50"
    -- local spineScale = 1.5
...
-- 更新佣兵碎片数量
function ActTimeLimit_116:updateMercenaryNumber()
...
function ActTimeLimit_116:getCurrentRoleNumber()
    for i = 1, #MercenaryRoleInfos do
        if _MercenaryInfo.itemId == MercenaryRoleInfos[i].itemId then
...
function ActTimeLimit_116:onBtnClick_1(container)
    ...
    common:sendPacket(HP_pb.RELEASE_UR_DRAW2_C, msg)
end

function ActTimeLimit_116:onBtnClick_2(container)
...
-- 积分抽奖事件
function ActTimeLimit_116:onScoreClick(container)
    ...
    common:sendPacket(HP_pb.RELEASE_UR_LOTTERY2_C, msg)
end

function ActTimeLimit_116:onIllustatedOpen(container)
    local FetterManager = require("FetterManager")
    FetterManager.showFetterPage(_MercenaryInfo.itemId)
end

function ActTimeLimit_116:onRewardPreview(container)
    ...
    NewSnowPreviewRewardPage_SetConfig(commonRewardItems, luckyRewardItems, "@ACTTLNewTreasureRaiderInfoTxt1", "@ACTTLNewTreasureRaiderInfoTxt2", GameConfig.HelpKey.HELP_ACT_116)
    PageManager.pushPage("NewSnowPreviewRewardPage")
end


function ActTimeLimit_116:createRewardItem(index)
...
TreasureRaiderPageNew = CommonPage.newSub(ActTimeLimit_116, thisPageName, option)

return ActTimeLimit_116