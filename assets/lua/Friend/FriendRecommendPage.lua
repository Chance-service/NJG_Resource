...
local tittleManager = require("PlayerInfo.TitleManager")
...
function FriendItem.onRefreshItemView(container)
	...
	for i=1, 2 do
		...
		if subContent then
            ...
            local username = friendInfo.name;
	        local length = GameMaths:calculateStringCharacters(username)
            local limitLen = GameConfig.FriendNameLenLimit.strLen;
	        if length > limitLen then
		        username = GameMaths:getStringSubCharacters(username, 0, limitLen).."..."
	        end
			-- set sub item's view
			local lb2Str = {
				mLevel 				= UserInfo.getOtherLevelStr(friendInfo.rebirthStage, friendInfo.level),
				mName 				= username,
				mFightValue 		= friendInfo.fightValue,
			}
			NodeHelper:setStringForLabel(contentContainer, lb2Str)
            NodeHelper:setLabelOneByOne(contentContainer,"mLevel","mName",5,true)
			--image
			--local icon ,bgIcon = common:getPlayeIcon(prof,0)
