...
--[[ 協定 ]]
Inst.opcodes["ACTIVITY146_CHOSEN_INFO_S"] = HP_pb.ACTIVITY146_CHOSEN_INFO_S
Inst.opcodes["ACTIVITY146_CHOSEN_DRAW_S"] = HP_pb.ACTIVITY146_CHOSEN_DRAW_S
...
function Inst:onReceivePacket(packet)
    ...
    if self:handleSummonError(packet, HP_pb.ACTIVITY146_CHOSEN_DRAW_C) == true then return end

    if packet.opcode == HP_pb.ACTIVITY146_CHOSEN_INFO_S or
       packet.opcode == HP_pb.ACTIVITY146_CHOSEN_DRAW_S then
        if packet.opcode == HP_pb.ACTIVITY146_CHOSEN_INFO_S then
            print("ACTIVITY146_CHOSEN_INFO_S")
        end
...
function Inst:sendRequestInfo (isShowLoading)
    ...
    if IS_MOCK then
        self:onReceivePacket({
            opcode = HP_pb.ACTIVITY146_CHOSEN_INFO_S,
            msg = {
                freeTimes = 0,
                onceCostGold = 120,
                tenCostGold = 1200,
    ...
    common:sendEmptyPacket(HP_pb.ACTIVITY146_CHOSEN_INFO_C, isShowLoading)
end
...
function Inst:sendSummon1 ()
    if IS_MOCK then
        self:onReceivePacket({
            opcode = HP_pb.ACTIVITY146_CHOSEN_DRAW_S,
            msg = {
                reward = "10000_1001_1",
            }
        })
    ...
    self.task = ALFManager:loadSpineTask(spineFolderAndName[1] .. "/", spineFolderAndName[2], texNum, function() 
       if not Inst.JumpAnim then
        ...
        msg.times = 1
        common:sendPacket(HP_pb.ACTIVITY146_CHOSEN_DRAW_C, msg, true)
