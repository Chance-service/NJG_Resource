...
function TitleManager:getTitleStrById( id )
	if TitleCfg[id] ~= nil then
		return TitleCfg[id].name
	end
end

function TitleManager:getTitleTypeById( id )
	if TitleCfg[id] ~= nil then
		return TitleCfg[id].type
	end
end
...
function TitleManager:setBMFontLabelTittle(node,id,fontSize,withBracket)
	if TitleCfg[id] ~= nil or id==0 then
        ...
        if node:getParent():getChildByTag(TTFTag) then 
