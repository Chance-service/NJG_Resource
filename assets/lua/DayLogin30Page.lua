...
function DayLogin30Page:refreshAll(container)
    ...
    leftSupplementaryCount = supplementaryCountTable[UserInfo.playerInfo.vipLevel] - self:getTableLen(serverData.supplSignedDays)
    ...
    local id = ActivityInfo.Day30Ids[1] or 113
...
function DayLogin30Page:registerPacket(container)
...
function DayLogin30Page:onReceivePacket(container)
...
--
function DayLogin30Page:removePacket(container)
    for key, opcode in pairs(opcodes) do
        if string.sub(key, -1) == "S" then
...
NewServerActivity = CommonPage.newSub(DayLogin30Page, thisPageName, option)
