...
function LeaveMessageDetailItem.onRefreshItemView( container )
	...
	local htmlNode = container:getVarLabelTTF("mMessage")
	--labMsg:setString( msg )
    local labMsg = NodeHelper:setCCHTMLLabelAutoFixPosition(htmlNode, CCSize(200,48),itemInfo.content )
	labMsg:setScaleX(htmlNode:getScaleX())
	labMsg:setScaleY(htmlNode:getScaleY())
...
function LeaveMessageDetailPageBase:onEnter( container )
    ...
    NewbieGuideManager.showHelpPage(GameConfig.HelpKey.HELP_LEAVEMSG_DETAIL)
end
