...
local thisPageName = "GamblingMachine"
...
local option = {
    ccbiFile = "Act_TimeLimitGamblingMachine.ccbi",
    handlerMap =
    {
...
local opcodes = {
    ...
    RELEASE_UR_RESET_C = HP_pb.RELEASE_UR_RESET_C,
    -- 重置积分奖池请求
    RELEASE_UR_RESET_S = HP_pb.RELEASE_UR_RESET_S,
    -- 重置积分奖池返回
    RELEASE_UR_LOTTERY_C = HP_pb.RELEASE_UR_LOTTERY_C,
    -- 积分抽奖请求
    RELEASE_UR_LOTTERY_S = HP_pb.RELEASE_UR_LOTTERY_S,-- 积分抽奖返回
}
...
GamblingMachine.timerName = "Activity_GamblingMachine"
GamblingMachine.timerLabel = "mTanabataCD"
GamblingMachine.timerKeyBuff = "Activity_GamblingMachine_Timer_Key_Buff"
GamblingMachine.timerFreeCD = "Activity_GamblingMachine_Timer_Free_CD"

function GamblingMachine:onEnter(parentContainer)
    math.randomseed(os.time())
    local container = ScriptContentBase:create(option.ccbiFile)
    self.container = container
    luaCreat_GamblingMachine(container)
...
function GamblingMachine:initData()
...
-- end
function GamblingMachine:initUi(container)
...
function GamblingMachine:getPoolRewardData(index)
    return mRollItemRewardData[index]
end

function GamblingMachine:setRollItemData(item, data)
...
-- 能不能更新积分奖池
function GamblingMachine:getIsCanRefreshScoreRewardPool()
    local bl = false
...
function GamblingMachine:formatRollItemData()
    local t = { }
    for k, v in pairs(TreasureRaiderDataHelper.TreasureRaiderConfig) do
...
function GamblingMachine:formatRewardData(data)
...
function GamblingMachine:formatRewardPopUpData(str)
...
function GamblingMachine:getTableLen(t)
    local index = 0
...
function GamblingMachine:getPageInfo(container)
...
-- 刷新UR抽奖界面
function GamblingMachine:refreshURPage(container)
    ...
    -- 设置节点是否显示
    NodeHelper:setNodesVisible(container, {
        mBtnFreeNode_1 = TreasureRaiderDataHelper.freeTreasureTimes <= 0,
...
-- 刷新积分抽奖按钮状态
function GamblingMachine:refreshScoreBtn(container)
    local bl = true
...
-- 刷新重置按钮状态
function GamblingMachine:refreshResetBtn(container)
...
-- 刷新页面
function GamblingMachine:refreshPage(container)
    self:refreshURPage(container)
end

-- 重置积分奖池
function GamblingMachine:resetScorePool()
...
-- 收包
function GamblingMachine:onReceivePacket(parentContainer)
    ...
    elseif opcode == HP_pb.RELEASE_UR_INFO_S or opcode == HP_pb.RELEASE_UR_DRAW_S then
        ...
        msg:ParseFromString(msgBuff)
        self:updateData(parentContainer, opcode, msg)
    elseif opcode == HP_pb.RELEASE_UR_RESET_S then
        -- 重置积分奖池返回
    ...
    elseif opcode == HP_pb.RELEASE_UR_LOTTERY_S then
        -- 积分抽奖返回
...
-- 显示积分抽奖动画
function GamblingMachine:showScoreAnimation(msg)
...
-- 刷新积分
function GamblingMachine:refreshScore(comluckey)
...
function GamblingMachine:updateData(parentContainer, opcode, msg)
...
function GamblingMachine:pushRewardPage()
    local data = { }
    ...
    if not CommonRewardAni:isPop() then
        ...
        --        CommonRewardAni:setFirstData(data, data.rewards, GamblingMachine.onBtnClick_1, GamblingMachine.onBtnClick_2, false, function()
        ...
        CommonRewardAni:setFirstDataNew(data.onceGold, data.tenGold, data.rewards, isFree, 0, "", false, GamblingMachine.onBtnClick_1, GamblingMachine.onBtnClick_2, function()
            if #TreasureRaiderDataHelper.reward == 10 then
                PageManager.showComment(true)
                -- 评价提示
            end
        ...
        CommonRewardAni:setFirstDataNew(data.onceGold, data.tenGold, data.rewards, isFree, 0, "", true, GamblingMachine.onBtnClick_1, GamblingMachine.onBtnClick_2)
        -- CommonRewardAni:setFirstData(data, data.rewards, GamblingMachine.onBtnClick_1, GamblingMachine.onBtnClick_2, true)
    end
end

function GamblingMachine:isContain(t, n)
    for k, v in pairs(t) do
        if v == n then
            return true
        end
...
function GamblingMachine:getLastScorePool(lotteryindex)
...
function GamblingMachine:popUpRewardPage(rewardItems)
...
function GamblingMachine:registerPacket(container)
...
function GamblingMachine:removePacket(container)
    for key, opcode in pairs(opcodes) do
        if string.sub(key, -1) == "S" then
...
function GamblingMachine:onExecute(parentContainer)
    self:onTimer(self.container)
end

function GamblingMachine:onTimer(container)
...
function GamblingMachine:onExit(parentContainer)
...
function GamblingMachine:initSpine(container)
    ...
    -- local spinePosOffset = "-130,-20"
    -- local spineScale = 0.35
...
-- 更新佣兵碎片数量
function GamblingMachine:updateMercenaryNumber()
...
function GamblingMachine:getCurrentRoleNumber()
    for i = 1, #MercenaryRoleInfos do
        if _MercenaryInfo.itemId == MercenaryRoleInfos[i].itemId then
...
function GamblingMachine:onBtnClick_1(container)
...
function GamblingMachine:onBtnClick_2(container)
...
-- 积分抽奖事件
function GamblingMachine:onScoreClick(container)
    ...
    common:sendPacket(HP_pb.RELEASE_UR_LOTTERY_C, msg)
end

-- 重置积分奖池事件  如果当前积分奖池没有被抽出去的  这个按钮是置灰
function GamblingMachine:onResetClick(container)
    if mIsAnimationInProgress then return end
    ...
    msg.times = 1
    common:sendPacket(HP_pb.RELEASE_UR_RESET_C, msg)
end

-- 抽奖次数随机 现在不用了
function GamblingMachine:onBtnClick_3(container)
    if LLL == nil then
        return
    end
...
function GamblingMachine:onIllustatedOpen(container)
    local FetterManager = require("FetterManager")
    FetterManager.showFetterPage(_MercenaryInfo.itemId)
end

function GamblingMachine:onRewardPreview(container)
    ...
    NewSnowPreviewRewardPage_SetConfig(commonRewardItems, luckyRewardItems, "@ACTTLNewTreasureRaiderInfoTxt1", "@ACTTLNewTreasureRaiderInfoTxt2", GameConfig.HelpKey.HELP_UR_CHOUKA)
    PageManager.pushPage("NewSnowPreviewRewardPage")
end


-- 得到奖励节点的开始位置   世界坐标 
function GamblingMachine:getRewardItemStratWorldPosition()
...
function GamblingMachine:convertToWorldSpace(node)
    if node then
        local x, y = node:getPosition()
...
-- 设置积分奖池已领取图片显隐
function GamblingMachine:setItemIsReceive(item, bl)
    if item == nil then
        return
    end
    local maskSprite = tolua.cast(item:getChildByTag(mItemTag.Mask), "CCSprite")
...
function GamblingMachine:createRewardItem(index)
...
function GamblingMachine:RunActionSpeedAdd(node, time, index)
...
function GamblingMachine:RunActionSpeedReduce(node, time, index)
    local delayTime = CCDelayTime:create(time)
    local callFunc = CCCallFunc:create( function()
        time = time + 0.1
        node:setPosition(mRewardPosition[TreasureRaiderDataHelper.lastRewardTable[index]])
...
function GamblingMachine:RunActionSpeedUniformity(node, time, index)
    local delayTime = CCDelayTime:create(time)
    local callFunc = CCCallFunc:create( function()
        node:setPosition(mRewardPosition[TreasureRaiderDataHelper.lastRewardTable[index]])
        -- node:setPosition(mRewardPosition[index])
...
function GamblingMachine:removeLastRewardTable(index)
    local t = { }
    for k, v in pairs(TreasureRaiderDataHelper.lastRewardTable) do
        if v == index then
...
function GamblingMachine:getMoveToIndex(currentIndex)
...
TreasureRaiderPageNew = CommonPage.newSub(GamblingMachine, thisPageName, option)
