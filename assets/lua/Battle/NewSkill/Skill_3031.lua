...
-------------------------------------------------------
function Skill_3031:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_3031:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_3031:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_3031:isUsable(chaNode, skillType, skillId, triggerType)
    local skillBaseId = math.floor(skillId / 10)
