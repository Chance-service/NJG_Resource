...
-------------------------------------------------------
--[[
對敵方全體造成150%/150%/150%/150%/150%(params1)傷害，並賦予"防禦破壞I"(params2)5秒(params3)
]]--
-------------------------------------------------------
function Skill_5509:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5509:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5509:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5509:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end
