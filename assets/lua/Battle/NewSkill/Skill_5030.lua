...
-------------------------------------------------------
--[[
對敵方全體造成400%/400%/400%/400%/400%(params1)傷害，並驅散目標身上Buff，
賦予友方全體"狂戰士"(params2)5秒(params3)
]]--
-------------------------------------------------------
function Skill_5030:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5030:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5030:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    ...
    for i = 1, #aliveIdTable do
        ...
        if NewBattleUtil:calIsHit(chaNode, target) then
            ...
            table.insert(spFuncTable, "clearAllBuff")
...
function Skill_5030:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end
