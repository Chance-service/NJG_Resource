...
-------------------------------------------------------
--[[ NEW
對隨機友方目標造成100%/100%/100%(params1)傷害，並獲得20/25/30MP(params2)
]]--
--[[ OLD
當自身周圍橢圓形區域(w:240(params1), h:150(params2))有敵人時，對當前目標造成280%/310%/350%(params3)傷害，並賦予"石化"(params4)2/2/3秒(params5)
]]--
-------------------------------------------------------
function Skill_1241:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_1241:calSkillTarget(chaNode, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local params = common:split(skillCfg.values, ",")
    return SkillUtil:getRandomTarget(chaNode, NgBattleDataManager_getFriendList(chaNode), 1, NewBattleConst.SKILL_TARGET_CONDITION.WITHOUT_SELF)
end
function Skill_1241:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_1241:isUsable(chaNode, skillType, skillId, triggerType)
    ...
    if #aliveIdTable <= 1 then  -- 沒有可攻擊目標
        return false
    end
    return true
end

function Skill_1241:getSkillTarget(chaNode, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local params = common:split(skillCfg.values, ",")
    return SkillUtil:getRandomTarget(chaNode, NgBattleDataManager_getFriendList(chaNode), 1, NewBattleConst.SKILL_TARGET_CONDITION.WITHOUT_SELF)
end
