...
-------------------------------------------------------
--[[
對當前目標造成140%(params1)傷害，並對"燃燒"(params2)中的目標額外賦予"暈眩"(params3)2秒(params4)
]]--
-------------------------------------------------------
function Skill_50016:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_50016:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_50016:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    ...
    resultTable = { }
    if not allTarget or #allTarget <= 0 then
        return resultTable
    end
    ...
    for i = 1, #allTarget do
        ...
        if NewBattleUtil:calIsHit(chaNode, target) then
            ...
            --觸發特效
            local trigger = self:triggerSpEffect(target, skillId)
            --最終結果
...
function Skill_50016:getSkillTarget(chaNode, skillId)
    return { chaNode.target }
end

function Skill_50016:triggerSpEffect(target, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local params = common:split(skillCfg.values, ",")
