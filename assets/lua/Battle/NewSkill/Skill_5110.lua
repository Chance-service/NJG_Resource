...
-------------------------------------------------------
--[[
對隨機敵方單體造成800%/800%/800%/800%/800%(params1)傷害，並有10%(params2)機率賦予"防禦破壞I"(params3)5秒(params4)
]]--
-------------------------------------------------------
function Skill_5110:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5110:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5110:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5110:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end

function Skill_5110:getSkillTarget(chaNode, skillId)
    local enemyList = NgBattleDataManager_getEnemyList(chaNode)
    return SkillUtil:getRandomTarget(chaNode, enemyList, 1)
end

function Skill_5110:isHitEffect(chaNode, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local params = common:split(skillCfg.values, ",")
