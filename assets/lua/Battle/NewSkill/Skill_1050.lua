...
-------------------------------------------------------
--[[ NEW
對目標造成每下150%/180%/210%(params1)傷害，並賦予自身"虔誠I/虔誠II/虔誠III"(params2)5/8/12秒(params3)
]]--
--[[ OLD
對魔力最高敵方單體造成每下275%/300%/325%(params1)傷害，並回復造成傷害50%/50%/100%的HP
]]--
-------------------------------------------------------
function Skill_1050:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_1050:calSkillTarget(chaNode, skillId)
    local enemyList = NgBattleDataManager_getEnemyList(chaNode)
    aliveIdTable = NewBattleUtil:initAliveTable(enemyList)
    local allTarget = self:getSkillTarget(chaNode, skillId)
    return allTarget
end
function Skill_1050:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_1050:getSkillTarget(chaNode, skillId)
