...
-------------------------------------------------------
--[[
對敵方全體造成240%/240%/240%/240%/240%(params1)傷害，並有30%(params2)機率賦予"自然印記I"(params3)5秒(params4)
]]--
-------------------------------------------------------
function Skill_5050:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5050:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5050:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5050:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end

function Skill_5050:isHitEffect(chaNode, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local params = common:split(skillCfg.values, ",")
