...
-------------------------------------------------------
--[[
對敵方單體造成240%/240%/240%/240%/240%(params1)傷害，並賦予"自然印記I"(params2)10秒(params3)
]]--
-------------------------------------------------------
function Skill_5505:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5505:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5505:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5505:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end

function Skill_5505:getSkillTarget(chaNode, skillId)
