...
-------------------------------------------------------
--[[ NEW
迴避成功後，對自身中心橢圓形區域(w:220(params1), h:120(params2))內敵人造成100%/120%/140%(params3)傷害
]]--
--[[ OLD
對自身中心橢圓形區域(w:220(params1), h:120(params2))內敵人造成130%/160%/190%(params3)傷害，並賦予自身"暴風I/暴風I/暴風II"(params4)5秒(params5)
]]--
-------------------------------------------------------
function Skill_1161:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_1161:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_1161:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_1161:isUsable(chaNode, skillType, skillId, triggerType)
    if triggerType ~= NewBattleConst.SKILL1_TRIGGER_TYPE.DODGE then
        return false
    end
...
function Skill_1161:getSkillTarget(chaNode, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local params = common:split(skillCfg.values, ",")
