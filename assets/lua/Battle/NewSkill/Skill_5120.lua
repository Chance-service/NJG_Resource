...
-------------------------------------------------------
--[[
對敵方全體造成150%/150%/150%/150%/150%(params1)傷害，並有10%(params2)機率賦予"狂亂"(params3)3秒(params4)
]]--
-------------------------------------------------------
function Skill_5120:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5120:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5120:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5120:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end

function Skill_5120:isHitEffect(chaNode, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local params = common:split(skillCfg.values, ",")
