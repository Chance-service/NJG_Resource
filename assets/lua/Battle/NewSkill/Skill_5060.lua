...
-------------------------------------------------------
--[[
對敵方全體造成240%/240%/240%/240%/240%(params1)傷害，並有30%(params2)機率賦予"沉默"(params3)5秒(params4)
]]--
-------------------------------------------------------
function Skill_5060:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5060:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5060:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5060:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end

function Skill_5060:isHitEffect(chaNode, skillId)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    local params = common:split(skillCfg.values, ",")
