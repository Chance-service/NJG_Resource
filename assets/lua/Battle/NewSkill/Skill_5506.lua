...
-------------------------------------------------------
--[[
對當前HP最低的敵方單體造成30%/30%/30%/30%/30%(params1)傷害，並賦予"暈眩"(params2)0.5秒(params3)
]]--
-------------------------------------------------------
function Skill_5506:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5506:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5506:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5506:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end

function Skill_5506:getSkillTarget(chaNode, skillId)
    local enemyList = NgBattleDataManager_getEnemyList(chaNode)
    return SkillUtil:getLowHpTarget(chaNode, enemyList, 1)
end
