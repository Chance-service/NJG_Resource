...
-------------------------------------------------------
--[[ NEW
隱身結束後對物理防禦力最低敵方單體造成120%/140%/160%(params1)傷害
]]--
--[[ OLD
觸發閃避後對該目標造成180%/210%/250%(params1)傷害，Lv3時賦予"沉默"(params2)3秒(param3)
]]--
-------------------------------------------------------
function Skill_1231:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_1231:calSkillTarget(chaNode, skillId)
    return scriptData[chaNode.idx] and scriptData[chaNode.idx][1]
end
function Skill_1231:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
    ...
    scriptData[chaNode.idx] = nil

    return resultTable
end
function Skill_1231:isUsable(chaNode, skillType, skillId, triggerType)
    if triggerType ~= NewBattleConst.SKILL1_TRIGGER_TYPE.STEALTH_CLEAR then
        return false
    end
...
function Skill_1231:getSkillTarget(chaNode, skillId)
