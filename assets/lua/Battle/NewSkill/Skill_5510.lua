...
-------------------------------------------------------
--[[
對敵方全體造成135%/135%/135%/135%/135%(params1)傷害，並賦予"侵蝕I"(params2)10秒(params3)
]]--
-------------------------------------------------------
function Skill_5510:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5510:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5510:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5510:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end
