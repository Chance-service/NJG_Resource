...
-------------------------------------------------------
--[[
對敵方全體造成440%/440%/440%/440%/440%(params1)傷害，並賦予"致盲I"(params2)10秒(params3)
]]--
-------------------------------------------------------
function Skill_5507:castSkill(chaNode, skillType, skillId)
    --紀錄skill data
...
-- 提前計算技能目標用 不使用時回傳nil
function Skill_5507:calSkillTarget(chaNode, skillId)
    return nil
end
function Skill_5507:runSkill(chaNode, skillId, resultTable, allPassiveTable, targetTable, attack_params)
    local skillCfg = ConfigManager:getSkillCfg()[skillId]
...
function Skill_5507:isUsable(chaNode, skillType, skillId, triggerType)
    return true
end
